...
export default async function viewHub() {
  ...
  return `
    ...
    <div class="grid grid-cols-2 xs:grid-cols-4 gap-md mb-lg">${stats.map(s=>`<div class="bg-card border border-border rounded p-md text-center"><div class="text-xl font-extrabold text-violet">${s.value}</div><div class="text-xs text-muted mt-xs">${s.label}</div></div>`).join('')}</div>
    <h2 class="text-xl font-extrabold tracking-tight mb-lg">Schnellzugriff</h2>
    <div class="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-3 gap-md">${links.map(item=>`<a href="#${item.id}" class="bg-card border border-border rounded p-lg flex flex-col gap-sm no-underline transition-all duration-base hover:-translate-y-0.5 hover:shadow-md hover:border-purple/30"><i data-lucide="${ICONS[item.id]||'circle'}" class="w-5 h-5 text-violet"></i><div class="text-base font-bold text-txt">${item.label}</div><div class="text-sm text-muted leading-snug">${DESCS[item.id]||''}</div></a>`).join('')}</div>
  `;
}
