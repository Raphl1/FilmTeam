...
export default async function viewKanban() {
  ...
  return `
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-md overflow-x-auto pb-sm">
      ${columns.map(col => {
        const ct = tasks.filter(t=>t.status===col.id);
        return `<div class="kanban-column flex flex-col bg-card2 rounded min-h-[200px] border border-border" data-column="${col.id}">
          <div class="flex items-center gap-sm p-md pb-sm"><div class="w-3 h-3 rounded-full shrink-0" style="background:${col.color}"></div><span class="text-sm font-bold text-txt">${col.label}</span><span class="text-xs font-semibold px-[8px] py-[2px] rounded-full bg-bg text-muted ml-auto">${ct.length}</span></div>
          <div class="kanban-col-body flex flex-col gap-sm p-sm flex-1 min-h-[100px]" data-col-id="${col.id}">
            ${ct.map(task => {
              const ri = tasks.indexOf(task);
              const del = editing ? `<button class="absolute top-sm right-sm w-5 h-5 flex items-center justify-center rounded-full bg-accent/10 text-accent text-xs cursor-pointer border-none hover:bg-accent/20" data-action="delete-kanban-task" data-idx="${ri}"><i data-lucide="x"></i></button>` : '';
              ...
              const at = task.assignee ? task.assignee.split(',').map(a=>`<span class="text-xs px-sm py-xs rounded-full bg-violet/10 text-violet">${escapeHtml(a.trim())}</span>`).join('') : '';
              ...
              const df = editing ? `<div class="text-xs text-muted leading-relaxed mt-xs" contenteditable="true" data-field="description" data-file="kanban" data-idx="${ri}" data-subkey="tasks">${escapeHtml(task.description||'')}</div>` : (task.description?`<div class="text-xs text-muted leading-relaxed mt-xs">${escapeHtml(task.description)}</div>`:'');
              ...
              return `<div class="kanban-card bg-card border border-border rounded-sm p-sm relative flex flex-col gap-xs transition-all duration-base hover:shadow-sm hover:border-purple/30 cursor-grab" ${draggable} data-task-idx="${ri}">
                ...
                <div class="text-sm font-bold text-txt leading-snug">${tf}</div>${df}
...
function onDragStart(e) { e.dataTransfer.setData('text/plain', e.currentTarget.dataset.taskIdx); e.dataTransfer.effectAllowed='move'; requestAnimationFrame(()=>e.currentTarget.classList.add('dragging')); }
...
function onDrop(e) { e.preventDefault(); const c=e.currentTarget,idx=parseInt(e.dataTransfer.getData('text/plain')),col=c.dataset.colId; cleanup(); if(!isNaN(idx)&&col&&state.kanban.tasks[idx]){const t=state.kanban.tasks[idx];if(t.status!==col){t.status=col;markDirty('kanban');navigate(getRoute(),true);}} }
...
let td={active:false,timer:null,startX:0,startY:0,card:null,ghost:null,taskIdx:null};
function initTouchDnD(cards) { cards.forEach(c=>{c.addEventListener('touchstart',onTS,{passive:false});c.addEventListener('touchmove',onTM,{passive:false});c.addEventListener('touchend',onTE,{passive:false});c.addEventListener('touchcancel',onTC,{passive:false});}); }
function onTS(e){if(e.touches.length!==1)return;const t=e.touches[0],c=e.currentTarget;td.startX=t.clientX;td.startY=t.clientY;td.card=c;td.taskIdx=c.dataset.taskIdx;td.timer=setTimeout(()=>startTD(c,t),500);}
function onTM(e){const t=e.touches[0],dx=Math.abs(t.clientX-td.startX),dy=Math.abs(t.clientY-td.startY);if(!td.active&&(dx>10||dy>10)){clearTimeout(td.timer);return;}if(td.active){e.preventDefault();if(td.ghost){td.ghost.style.left=(t.clientX-td.ghost.offsetWidth/2)+'px';td.ghost.style.top=(t.clientY-20)+'px';}cleanup();const eb=document.elementFromPoint(t.clientX,t.clientY);const cb=eb?.closest('.kanban-col-body');if(cb){cb.classList.add('drag-over');placeIndicator(cb,t.clientY);}}}
function onTE(e){clearTimeout(td.timer);if(td.active){e.preventDefault();const t=e.changedTouches[0];if(td.ghost)td.ghost.style.display='none';const eb=document.elementFromPoint(t.clientX,t.clientY);if(td.ghost)td.ghost.style.display='';const cb=eb?.closest('.kanban-col-body');if(cb){const col=cb.dataset.colId,idx=parseInt(td.taskIdx);if(!isNaN(idx)&&col&&state.kanban.tasks[idx]){const tk=state.kanban.tasks[idx];if(tk.status!==col){tk.status=col;markDirty('kanban');}}}cancelTD();navigate(getRoute(),true);}else{cancelTD();}}
function onTC(){clearTimeout(td.timer);cancelTD();}
function startTD(c,t){td.active=true;c.classList.add('dragging');const g=c.cloneNode(true);g.className='kanban-card kanban-drag-ghost';g.style.cssText=`width:${c.offsetWidth}px;left:${t.clientX-c.offsetWidth/2}px;top:${t.clientY-20}px`;document.body.appendChild(g);td.ghost=g;if(navigator.vibrate)navigator.vibrate(30);}
function cancelTD(){td.ghost?.remove();td.card?.classList.remove('dragging');cleanup();td.active=false;td.card=null;td.ghost=null;td.taskIdx=null;}
